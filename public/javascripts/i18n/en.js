$.eatLang.en = {
    adddish : 'Add dish',
    addmeal : 'Book meal!',
    aftertomorrow : 'After tomorrow',
    alertcontinue : 'Continue',
    alreadyjoined : 'Already joined a group',
    alreadybookedtext : 'You already booked a meal',
    allportionsbooked : 'All portions booked!',
    bookedmeals : 'Who booked a portion?',
    cancel : 'Cancel',
    capturephoto : 'Take photo',
    chef : 'Chef',
    close : 'Close',
    confirmadddish : 'Add dish and notify group members?',
    confirmbook : 'Book dish and notify the chef?',
    continue : 'Continue',
    deleteduser : 'The user does not exist anymore',
    dish : 'Dish',
    dishadded : 'Dish added',
    dishaddedfirstinvites : 'You can invite people to join your group',
    dishaddednomembersinfo : 'You are the only group member! Invite to your group the colleagues you want to share the dish with',
    dishdescription : 'Description',
    dishedited : 'Dish edited',
    dishes : 'Dishes list',
    dishname : 'Dish name',
    editdish : 'Edit dish',
    editprofile : 'Edit profile',
    email : 'Email',
    error : 'Error',
    expecteddonation : 'Expected donation',
    groupcreated : 'Welcome',
    groupcreatedinfo : 'Time to share dishes! Invite colleagues to join your group, you can do it though Participants (link in the menu) or after adding your first dish.',
    groupexists : 'Group name already used',
    groupexistsinfo : 'Select another group name. This one is already used.',
    groupnoexists : 'The group does not exist',
    ibooked : 'Booked',
    icook : 'I cook',
    insertgroupcode : 'Group code',
    inviteemailsubject : 'I\'ve cooked a dish I\'d like to share with you',
    invitegroupcode : 'The group code is',
    invitenew : 'Invite',
    inviteimcooking : 'Hi, I\'m cooking',
    invitejoindetailsbook1 : 'Join my http://eat-this-one.com group',
    invitejoindetailsbook2 : 'to see the details and book a portion',
    invitejoinmygroup1 : 'Hi, join my http://eat-this-one.com group',
    invitejoinmygroup2 : 'to share meals at work or at the uni during lunch time',
    joinedgroupinfo : 'You will receive notifications when other members add new dishes and when they book yours.',
    joingroup : 'I received an invitation to join a group',
    lastportion : 'Last portion!',
    lnbooked : 'booked',
    lnchef : 'Chef',
    lncooked : 'cooked',
    lnjoined : 'joined your group!',
    lnremember : 'Remember it!',
    loading : 'Loading',
    membersince : 'Group member since',
    menu : 'Menu',
    message : 'Message',
    missingfields : 'Hey!',
    missingfieldsinfo : 'Some fields need your attention.',
    name : 'Name',
    newgroup : 'I want to create a new group',
    nportions : 'Available portions',
    nodishes : 'No meals to book in the next days yet. Why don\'t you add the first one?',
    nothing : 'Nothing',
    notnow : 'Not now',
    notyet : 'Not yet',
    open : 'Open',
    participants : 'Participants',
    refresh : 'Refresh',
    save : 'Save',
    selectimage : 'Photos gallery',
    setcountry : 'and country where you usually have lunch',
    setgroup : 'Set your group',
    setgroupname : 'Set a group name',
    signup : 'Sign up',
    sitename : 'Eat this one!',
    sure : 'Sure!',
    thanks : 'Thanks',
    tipadddish : 'Press to add a dish',
    tipbook : 'Press to book',
    tipcontinue : 'Press to continue',
    tipeditdish : 'Press to edit',
    tipsubmit : 'Press to submit',
    tipshare : 'Invite colleagues',
    today : 'Today',
    tomorrow : 'Tomorrow',
    unlimited : 'Unlimited',
    unlimitedselected : 'You selected unlimited portions option. When you don\'t want to receive more meal bookings change this value.',
    useralreadyingroup : 'You can only be member of one group',
    username : 'Your name',
    users : 'users',
    weird : 'There is no internet connection, try it later',
    welcome : 'Welcome',
    when : 'When',
    wrongcode : 'Wrong code',
    wrongcodeinfo : 'The provided code does not match any group.'
};
