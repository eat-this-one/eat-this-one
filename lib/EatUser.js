...
EatUser.prototype.prepareUserToSave = function(error, userObj, user) {
    ...
    if (!user) {
        ...
        for (var index in userObj) {
            if (userObj.hasOwnProperty(index)) {
...
EatUser.prototype.updateApntoken = function() {

    var apntoken = this.eat.getParam(
        'apntoken',
        this.attrs.apntoken.validation,
        this.attrs.apntoken.pattern
    );
    if (apntoken === false) {
        return this.eat.returnCallback({
            code: 400,
