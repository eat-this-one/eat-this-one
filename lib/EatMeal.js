...
EatMeal.prototype.add = function() {
    ...
    var dishCallback = function(error, dishData) {
        ...
        if (dishData.user._id.equals(this.eat.getUserid())) {
            return this.eat.returnCallback({
                code : 400,
        ...
        // Copying the dish data to wider scope var so membershipCallback can read it.
        dish = dishData;

        var membershipFilter = {
            user : meal.user,
            group : dish.group._id
        };
        this.eat.getEatGroupMember().fetch(membershipFilter, membershipCallback);

    }.bind(this);

    // Here we will check that the user is a dish group member.
    var membershipCallback = function(error, memberships) {
        if (error) {
            return this.eat.returnCallback(error);
        }
...
// TODO If we accept multiple groups we should also
// filter by the group here.
    ...
    meal.save(function(error) {
        ...
        var currentUserCallback = function(error, users) {
            ...
            var msgdata = {
                ...
                type: "newmeal",
                objectid: dish._id.valueOf(),
                url: "dishes/view.html?id=" + dish._id.valueOf()
            };
