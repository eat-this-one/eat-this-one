...
EatLocationSubscription.prototype.add = function() {
    ...
    var subscriptionExistsCallback = function(error, isMember) {
        ...
        var saveCallback = function(error) {
            ...
            // Notify the group members that there is a new subscription.
            LocationSubscriptionModel
                .find({loc: subscribedLocation._id})
                .populate('user')
                .exec(function(error, subscriptions) {
                    if (error) {
                        // Just log it as the dish is already created.
                        log('error', 'We can not get ' + subscribedLocation._id + ' group members', this.eat, error);
                        return this.eat.returnCallback(null, subscribedLocation, 201);
                    }

                    if (subscriptions.length === 0) {
                        // Log it, this is weird...
                        log('error', 'No ' + subscribedLocation._id + ' group members', this.eat, error);
                        return this.eat.returnCallback(null, subscribedLocation, 201);
                    }
                    ...
                    // We notify all the subscriptions that a new member joined.
                    var pusher = require('../lib/pusher.js');
                    pusher.sendNotifications(
                        this.eat,
                        users,
                        "newmember",
                        subscribedLocation._id,
                        "locations/view.html?id=" + subscribedLocation._id
                    );
