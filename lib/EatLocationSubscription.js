...
EatLocationSubscription.prototype.getMembers = function getMembers(error, locationid, callback) {
    ...
    // We need to wrap it as we don't want to return user sensitive data.
    var cleanUserWrapperCallback = function cleanUserWrapperCallback(error, members) {
        if (error) {
            return callback(error);
        }
        ...
        for (var i in members) {
            cleanMembers.push(this.eat.getEatUser().clearPersonalData(members[i]));
        }
    ...
    LocationSubscriptionModel
        .find({loc: locationid})
        .populate('user')
        .exec(cleanUserWrapperCallback.bind(this));
};
