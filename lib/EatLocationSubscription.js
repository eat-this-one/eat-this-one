    ...
    var subscriptionExistsCallback = function(error, isMember) {
        ...
        var saveCallback = function(error) {
            ...
            LocationSubscriptionModel
                .find({loc: subscribedLocation.loc})
                .populate('user')
                .exec(function(error, users) {
                    if (error) {
                        // Just log it as the dish is already created.
                        log('error', 'We can not get ' + subscribedLocation.loc + ' group members', this.eat, error);
                        return this.eat.returnCallback(null, subscribedLocation, 201);
                    }

                    if (users.length === 0) {
                        // Log it, this is weird...
                        log('error', 'No ' + subscribedLocation.loc + ' group members', this.eat, error);
                        return this.eat.returnCallback(null, subscribedLocation, 201);
                    }

                    // We notify all the users that a new member joined.
