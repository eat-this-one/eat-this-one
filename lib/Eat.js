...
Eat.prototype.returnCallback = function(error, returnData, statusCode) {
    if (error) {
        ...
        // We are forwarding here mongodb messages, we want all of them
        // to be 500 and keep the error as internal.
        if (error.code > 505) {
            log('error', 'Mongo DB error', this, error);
            error.code = 500;
...
Eat.prototype.generateNewToken = function(userid, callback) {
    ...
    var token = new TokenModel(tokenData);
...
Eat.prototype.getParam = function(key, validations, pattern) {
    ...
    for (var index in validations) {
        if (validations.hasOwnProperty(index)) {
...
Eat.prototype.getStringRegEx = function() {
    // Including accents.
    return /^[A-Za-z\u00C0-\u017F0-9-+_=".',;!?\s]*$/;
};

Eat.prototype.getApnRegEx = function() {
    return /[0-9a-zA-Z.]*/;
};
...
Eat.prototype.getEatGroupMember = function() {
    if (this.eatGroupMember === null) {
        var EatGroupMember = require('./EatGroupMember.js');
        this.eatGroupMember = new EatGroupMember(this);
    }
    return this.eatGroupMember;
};

Eat.prototype.getEatGroup = function() {
    if (this.eatGroup === null) {
        var EatGroup = require('./EatGroup.js');
        this.eatGroup = new EatGroup(this);
    }
